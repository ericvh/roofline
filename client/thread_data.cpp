...
ThreadData::ThreadData(int thread_id) {
  ...
  buf_base = reinterpret_cast<mem_ref_t *>(dr_raw_mem_alloc(
      MEM_BUF_SIZE, DR_MEMPROT_READ | DR_MEMPROT_WRITE, nullptr));
...
void ThreadData::clean_buffer(void) { BUF_PTR(seg_base) = buf_base; }

// Saves the point pushing it to the point list.
void ThreadData::save_point(std::string label, unsigned int line,
                            std::string src_file) {
...
// Saves the point pushing it to the point list.
void ThreadData::new_point(std::string label, unsigned int line,
                           std::string src_file) {
...
void ThreadData::save_to_file(file_t out_file) {
  ...
  for (std::list<Point>::iterator it = point_list.begin();
       it != point_list.end(); it++) {
