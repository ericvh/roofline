...
// TODO: This function can be improved way better using the DynamoRIO API. You
// should try differnt API calls. This functions checks out whether the given
// instruction is a FP one or not. by taking into account what kind of registers
// it's using.
...
uint32_t count_fp_instr(instr_t *instr) {
  ...
  if (operations_per_instr > 0) {
    if (is_vector_instruction(instr)) {  /* THIS ONLY HANDLES NEON */
      ...
      DR_ASSERT_MSG(
          opnd_is_immed_int(width_operand),
          "ERROR: Roofline Client - I'm expecting immediated value specifying "
          "width of elements at the end of NEON instruction\n");
...
// TODO: For X86, this is a really poor analysis, you're missing out all the
// benefits from vectorization and having instructions that fuse together
// multiple operations, such multiply/add
#ifdef FLOATING_POINTS_X86
uint32_t count_fp_instr(instr_t *instr) {
